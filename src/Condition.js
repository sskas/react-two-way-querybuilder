            ...
            <div className={this.styles.condition}>
                {this.props.nodeName !== '1'
                    ? <header>
                        ...
                        <button
                            ...
                            className={this.styles.deleteBtn}><svg><use xlinkHref="#queryicon-remove" /></svg>
                        </button>
                    </header>
                    : null}
                    {this.state
                    .data
                    .rules.length < 1? <p>Click add button or add group to build query</p>: null}
                {this.state
                    .data
                    .rules
                    .map((rule, index) => {
                ...
                <footer>
                    <button type="button" onClick={this.addRule}>
                        <svg><use xlinkHref="#queryicon-add" /></svg>
                    </button>
                    <button type="button" onClick={this.addCondition}>
                        <svg><use xlinkHref="#queryicon-add-group" /></svg>
                    </button>
                </footer>
            </div>
        );
